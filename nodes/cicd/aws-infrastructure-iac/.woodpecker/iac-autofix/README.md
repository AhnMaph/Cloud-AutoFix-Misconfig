...
python autofix.py \
  ...
  --tfsec   ./tfsec_output.json \ # đổi format chỗ này chưa sửa README
...
steps:
  scan:
    ...
    commands:
      - checkov -d . --output json > checkov_output.json || true

  autofix:
    image: python:3.11
    commands:
      - pip install -q colorama
      - python autofix.py --tf main.tf --checkov checkov_output.json
          --tfsec tfsec_output.json --trivy trivy_output.json
          --out-dir fix_output --severity LOW
